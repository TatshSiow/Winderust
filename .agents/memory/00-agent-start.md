...
- Main UI: `src/app.rs`.
- Background worker and status fan-out: `src/automation.rs`.
- Process add/check helpers are near the bottom of `src/app.rs`: `can_add_*`, `new_*_rule`, `new_process_exclusion_rule`.
...
- Auto-exclusion fallback is shared through `PendingAutoExclusions` in `src/automation.rs`.
...
- Mild watch item: static UI motion maps in `src/app.rs` keyed by string IDs; fine if IDs stay bounded/fixed.
