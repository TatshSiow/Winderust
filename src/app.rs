...
impl WinderustApp {
    ...
    fn render_process_list_column_visibility_dropdown(
        &self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> AnyElement {
...
fn setting_group_action_row_with_help(
    id: impl Into<SharedString>,
    title: impl Into<SharedString>,
    help: impl Into<SharedString>,
    action: AnyElement,
...
fn setting_group_stepper_row_u64_with_help(
    id: impl Into<SharedString>,
    title: impl Into<SharedString>,
    help: impl Into<SharedString>,
    value: u64,
...
fn setting_group_stepper_row_u64_inner(
    id: impl Into<SharedString>,
    title: impl Into<SharedString>,
    help: Option<SharedString>,
    value: u64,
...
const PROCESS_LIST_NAME_MAX_WIDTH: f32 = 340.0;
const PROCESS_LIST_PID_MIN_WIDTH: f32 = 56.0;
const PROCESS_LIST_PID_MAX_WIDTH: f32 = 90.0;
const PROCESS_LIST_COLUMN_MIN_WIDTH: f32 = 72.0;
const PROCESS_LIST_COLUMN_MAX_WIDTH: f32 = 250.0;
const PROCESS_LIST_TEXT_CELL_HORIZONTAL_PADDING: f32 = 24.0;
const PROCESS_LIST_NAME_CELL_NON_TEXT_WIDTH: f32 = 76.0;
const PROCESS_LIST_SORT_ICON_WIDTH: f32 = 18.0;
const PROCESS_LIST_SORT_HEADER_GAP: f32 = 4.0;
const PROCESS_LIST_SPLIT_LABEL_WIDTH: f32 = 18.0;
const PROCESS_LIST_SPLIT_LABEL_GAP: f32 = 4.0;
const PROCESS_LIST_CELL_EDITOR_WIDTH: f32 = 220.0;
...
const PROCESS_LIST_TREE_TOGGLE_WIDTH: f32 = 16.0;
const PROCESS_LIST_COLUMN_VISIBILITY_DROPDOWN_ID: &str = "process-list-column-visibility";
const PROCESS_LIST_COLUMN_VISIBILITY_DROPDOWN_WIDTH: f32 = 360.0;
...
const PROCESS_LIST_OPTIONAL_COLUMNS: [ProcessListColumn; 14] = [
...
enum ProcessListSortColumn {
    ProcessName,
    Column(ProcessListColumn),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum ProcessListSortDirection {
    Ascending,
    Descending,
}
...
impl ProcessListColumnLayout {
    fn column_width(&self, column: ProcessListColumn) -> f32 {
        self.column_widths
            .get(&column)
            .copied()
...
#[derive(Clone, Copy)]
struct ProcessListRenderLayout<'a> {
    hidden_columns: &'a HashSet<ProcessListColumn>,
    column_layout: &'a ProcessListColumnLayout,
}

#[derive(Clone, Copy)]
struct ProcessListGroupRowState {
    collapsed: bool,
    divided: bool,
}

#[derive(Clone, Copy)]
struct ProcessListEntryRowState {
    divided: bool,
    nested: bool,
    editable: bool,
}
...
#[derive(Clone, Copy)]
struct ProcessListPolicyCellTarget<'a> {
    process_name: &'a str,
    column: ProcessListColumn,
    editable: bool,
}
...
fn process_list_group_sort_cmp(
...
fn process_list_process_sort_cmp(
    left: &ProcessInfo,
    right: &ProcessInfo,
    sort: ProcessListSort,
) -> CmpOrdering {
...
fn process_list_directional_cmp(
...
fn process_list_group_min_pid(group: &ProcessListGroup<'_>) -> u32 {
    group
        .processes
        .iter()
...
fn process_list_group_sort_pid(
...
fn process_list_group_key(process_name: &str) -> String {
    process_name.trim().to_ascii_lowercase()
}

fn process_list_column_visible(
...
fn process_list_column_min_width(column: ProcessListColumn) -> f32 {
    match column {
...
fn process_list_column_max_width(column: ProcessListColumn) -> f32 {
    match column {
...
fn process_list_column_label(column: ProcessListColumn, settings: &Settings) -> String {
    match column {
...
fn process_list_estimated_cell_width(text: &str, extra_width: f32) -> f32 {
    process_list_estimated_text_width(text) + extra_width
}

fn process_list_estimated_policy_value_width(column: ProcessListColumn, value: &str) -> f32 {
...
fn process_list_header_cell_non_text_width() -> f32 {
    PROCESS_LIST_TEXT_CELL_HORIZONTAL_PADDING
        + PROCESS_LIST_SORT_ICON_WIDTH
        + PROCESS_LIST_SORT_HEADER_GAP
}

fn process_list_estimated_text_width(text: &str) -> f32 {
    text.chars().map(process_list_estimated_char_width).sum()
}

fn process_list_estimated_char_width(character: char) -> f32 {
...
fn process_list_surface() -> gpui::Div {
    v_flex()
        .size_full()
        .min_w(px(0.0))
        .min_h(px(0.0))
...
fn process_list_scroll_content(table_width: Pixels) -> gpui::Div {
    v_flex().w(table_width).min_w(table_width)
}

fn process_list_header_row(
    settings: &Settings,
    hidden_columns: &HashSet<ProcessListColumn>,
    layout: &ProcessListColumnLayout,
    sort: ProcessListSort,
...
fn process_list_priority_header_label(
    label: String,
...
fn process_list_foreground_short_label() -> &'static str {
    "FG"
}

fn process_list_background_short_label() -> &'static str {
    "BG"
}

fn process_list_header_cell(
    width: f32,
    label: String,
    column: ProcessListSortColumn,
    sort: ProcessListSort,
...
fn process_list_sort_column_id(column: ProcessListSortColumn) -> &'static str {
    match column {
...
fn process_list_column_visibility_dropdown_options(
    hidden_columns: &HashSet<ProcessListColumn>,
    settings: &Settings,
    max_height: Pixels,
    cx: &mut Context<WinderustApp>,
...
fn process_list_empty_row(message: impl Into<SharedString>) -> gpui::Div {
    h_flex()
        .w_full()
        .min_w(px(0.0))
        .h(px(CARD_ROW_HEIGHT))
...
fn process_list_group_name_cell(
    process_name: &str,
    process_count: usize,
    icon: Option<&Arc<Image>>,
    collapsed: bool,
...
fn process_list_text_cell(width: f32, value: impl Into<SharedString>) -> gpui::Div {
    process_list_text_cell_with_color(width, value, false, muted_text_color())
}

fn process_list_text_cell_with_color(
    width: f32,
...
fn process_list_policy_value_content(
...
fn process_list_split_policy_value_row(
    label: &'static str,
    value: &str,
    emphasized: bool,
    text_color: u32,
...
fn process_list_column_uses_split_priority_display(column: ProcessListColumn) -> bool {
    matches!(
        column,
        ProcessListColumn::IoPriority
            | ProcessListColumn::GpuPriority
...
fn process_list_split_policy_value(value: &str) -> Option<(&str, &str)> {
    let (foreground, background) = value.split_once(" / ")?;
    Some((foreground.trim(), background.trim()))
}

fn process_list_policy_cell(
    width: f32,
...
fn process_list_editable_policy_cell(
    width: f32,
    process_name: &str,
    column: ProcessListColumn,
    value: SharedString,
...
fn process_list_policy_cell_editable(row_editable: bool, column: ProcessListColumn) -> bool {
    row_editable && process_list_column_editable(column)
}

fn process_list_cell_editor_id(process_name: &str, column: ProcessListColumn) -> String {
    format!(
...
fn process_list_include_exclude_editor_options(
...
fn process_list_editor_option_id(
    process_name: &str,
    column: ProcessListColumn,
...
fn process_list_power_plan_editor_option(
    process_name: &str,
    column: ProcessListColumn,
    label: String,
    selected: bool,
...
fn process_list_include_exclude_editor_option(
    process_name: &str,
    column: ProcessListColumn,
    included: bool,
    selected: bool,
...
fn process_list_policy_value_active(value: &str, emphasized: bool) -> bool {
...
fn process_list_policy_value_inactive(value: &str) -> bool {
...
fn process_list_state_enabled(value: &str) -> Option<bool> {
    match value {
...
fn set_timer_resolution_override(
...
fn set_app_suspension_override(
...
fn default_process_policy_summary() -> ProcessPolicySummary {
    ProcessPolicySummary {
...
fn power_plan_policy_value_label(plans: &[PowerPlan], guid: Option<&str>) -> String {
...
fn process_list_include_exclude_label(included: bool) -> String {
    if included {
...
fn process_list_include_value_label(value: impl std::fmt::Display) -> String {
    format!("{} ({value})", process_list_include_label())
}

fn process_list_include_label() -> String {
    "Include".to_owned()
}

fn process_list_exclude_label() -> String {
    "Exclude".to_owned()
}

fn process_list_default_label() -> String {
...
fn io_priority_has_foreground_background_split(settings: &IoPrioritySettings) -> bool {
    settings.enabled
        && settings.foreground_detection_enabled
        && settings.foreground_priority != settings.background_priority
}

fn io_priority_policy_label(settings: &IoPrioritySettings) -> String {
...
fn gpu_priority_has_foreground_background_split(settings: &GpuPrioritySettings) -> bool {
    settings.enabled
        && settings.foreground_detection_enabled
        && settings.foreground_priority != settings.background_priority
}

fn gpu_priority_policy_label(settings: &GpuPrioritySettings) -> String {
...
fn memory_priority_has_foreground_background_split(settings: &MemoryPrioritySettings) -> bool {
    settings.enabled
        && settings.foreground_detection_enabled
        && settings.foreground_priority != settings.background_priority
}

fn memory_priority_policy_label(settings: &MemoryPrioritySettings) -> String {
