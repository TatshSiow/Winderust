...
impl WinderustApp {
    ...
    pub(in crate::ui::app) fn render_priority_exclusion_table(
        ...
        for (index, rule) in rules.iter().enumerate() {
            ...
            for tier in ProcessRuleTier::ALL {
                row = row
                    .child(self.render_priority_exclusion_dropdown(kind, index, tier, window, cx));
            }
            let row = row.child(rule_table_action_cell(
        ...
        match kind {
            ListItemRemovalKind::ProcessPriorityExclusion => {
                let selected = self.settings.process_priority.exclusions[index]
                    .process_priority_override(foreground, visible_window);
            ...
            ListItemRemovalKind::ThreadPriorityExclusion => {
                let selected = self.settings.thread_priority.exclusions[index]
                    .thread_priority_override(foreground, visible_window);
            ...
            ListItemRemovalKind::DynamicPriorityBoostExclusion => {
                let selected = self.settings.dynamic_priority_boost.exclusions[index]
                    .dynamic_priority_boost_override(foreground, visible_window);
                self.render_priority_rule_dropdown(
            ...
            ListItemRemovalKind::IoPriorityExclusion => {
                let selected = self.settings.io_priority.exclusions[index]
                    .io_priority_override(foreground, visible_window);
            ...
            ListItemRemovalKind::GpuPriorityExclusion => {
                let selected = self.settings.gpu_priority.exclusions[index]
                    .gpu_priority_override(foreground, visible_window);
            ...
            ListItemRemovalKind::MemoryPriorityExclusion => {
                let selected = self.settings.memory_priority.exclusions[index]
                    .memory_priority_override(foreground, visible_window);
                self.render_priority_rule_dropdown(
    ...
    fn render_priority_rule_dropdown<T>(
        ...
        set: impl Fn(&mut Self, usize, ProcessRuleTier, T) + Copy + 'static,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> AnyElement
    where
