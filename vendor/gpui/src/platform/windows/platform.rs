        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                loop {
                    ...
                    let all_windows_hidden = windows
                        .read()
                        .iter()
                        // SAFETY: each handle is owned by GPUI's live window registry.
                        .all(|hwnd| unsafe { !IsWindowVisible(hwnd.as_raw()).as_bool() });
                    if all_windows_hidden {
                        // ponytail: poll at 4 Hz; replace with a visibility event only if restore
